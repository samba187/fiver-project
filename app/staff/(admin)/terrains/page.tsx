...
import { LayoutGrid, CheckCircle, Wrench, ChevronLeft, ChevronRight, X as XIcon, DollarSign, MessageCircle, Eye, Calendar, Clock, MapPin, User, Receipt, Smartphone, CreditCard, Banknote } from "lucide-react";
...
const PAYMENT_METHODS = [
  { value: "bankily", label: "Bankily", icon: Smartphone, className: "bg-yellow-500/10 text-yellow-400" },
  { value: "masrvi", label: "Masrvi", icon: CreditCard, className: "bg-purple-500/10 text-purple-400" },
  { value: "especes", label: "Espèces", icon: Banknote, className: "bg-green-500/10 text-green-400" },
];
...
export default function TerrainsPage() {
  ...
  async function submitPayment() {
    if (payModal.id && payReceiver.trim() && payAmount) {
      ...
      await supabase.from("reservations").update({
        status: "paid", payment_receiver: payReceiver,
        payment_method: payMethod || res?.payment_method || "especes",
        amount_paid: newTotal, total_price: totalPrice, payment_confirmed: true,
      }).eq("id", payModal.id);
      setPayModal({ isOpen: false, id: null, res: null }); setPayReceiver(""); setPayAmount(""); setPayMethod("");
      fetchData();
    }
  }
  ...
  async function submitCancel() {
    if (cancelModal.id) {
      ...
      setCancelModal({ isOpen: false, id: null }); setCancelReason("");
      fetchData();
    }
  }
                    ...
                    <div className="flex flex-col gap-2">
                      {selectedDayReservations.map(r => (
                        <div key={r.id} onClick={() => openDetail(r)}
                          className="cursor-pointer rounded-sm border border-white/5 bg-white/[0.02] p-2.5 transition-all duration-200 hover:border-fiver-green/20 hover:bg-white/[0.04] active:scale-[0.98]">
            ...
            <div className="mx-6 mt-4 rounded-lg border border-white/5 bg-white/[0.03] p-4">
              ...
              <div className="mb-2 h-2 w-full overflow-hidden rounded-full bg-white/5">
                <div className={cn("h-full rounded-full transition-all duration-500",
                  ...
                  progressPercent > 0 ? "bg-gradient-to-r from-amber-500 to-amber-400" : "bg-white/10"
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4 backdrop-blur-sm animate-in fade-in duration-200">
        <div className="w-full max-w-sm rounded-lg border border-white/10 bg-[#161616] p-6 shadow-2xl relative">
          ...
          <input type="text" placeholder="Ex: Amadou" value={payReceiver} onChange={(e) => setPayReceiver(e.target.value)}
            className="mb-6 w-full rounded-sm border border-white/10 bg-white/5 px-4 py-3 text-sm text-white placeholder:text-white/30 focus:border-fiver-green focus:outline-none focus:ring-1 focus:ring-fiver-green" autoFocus />
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4 backdrop-blur-sm animate-in fade-in duration-200">
        <div className="w-full max-w-sm rounded-lg border border-white/10 bg-[#161616] p-6 shadow-2xl relative">
          ...
          <input type="text" placeholder="Ex: Le client n'est pas venu" value={cancelReason} onChange={(e) => setCancelReason(e.target.value)}
            className="mb-6 w-full rounded-sm border border-white/10 bg-white/5 px-4 py-3 text-sm text-white placeholder:text-white/30 focus:border-red-500 focus:outline-none focus:ring-1 focus:ring-red-500" autoFocus />
          <div className="flex justify-end gap-3">
